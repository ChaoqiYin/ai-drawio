---
name: ai-drawio-cli
description: Use when the task must be performed through the installed `ai-drawio` command, including opening the app, checking status, creating conversations, listing or opening sessions, reading the current canvas XML, exporting SVG pages, applying XML updates, or restoring a document. Use when the agent should prefer the shortest valid command and avoid optional `--` flags unless the user explicitly requires them.
---
...
Use this skill when operating the installed `ai-drawio` command directly. Prefer the smallest command that satisfies the request, and do not add optional `--` flags unless the user explicitly asks for the extra targeting, output, or metadata behavior.
