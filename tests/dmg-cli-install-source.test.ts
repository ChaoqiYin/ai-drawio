...
const DMG_BUILD_SCRIPT_PATH = new URL("../scripts/build-macos-cli-dmg.sh", import.meta.url);
...
test("macOS dmg build route uses the tauri dmg bundler directly", async () => {
  const dmgBuildScript = await readFile(DMG_BUILD_SCRIPT_PATH, "utf8");

  assert.match(dmgBuildScript, /--bundles dmg/);
  assert.doesNotMatch(dmgBuildScript, /hdiutil create/);
  assert.doesNotMatch(dmgBuildScript, /--bundles app/);
});
