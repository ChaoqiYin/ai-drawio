...
test("home page links to settings and settings page renders cli integration actions", async () => {
  ...
  assert.match(settingsSource, /IconToBottom/);
  assert.match(settingsSource, /icon=\{<IconToBottom \/>\}/);
  assert.match(settingsSource, /loading=\{isTogglingTray\}/);
  assert.match(settingsSource, /onClick=\{handleEnterTray\}/);
  assert.match(settingsSource, /disabled=\{isLoadingTray \|\| isTogglingTray \|\| traySettings\.enabled\}/);
  assert.match(settingsSource, />\s*进入托盘\s*<\/Button>/);
  assert.match(settingsSource, /await setTrayEnabled\(true\)/);
  assert.doesNotMatch(settingsSource, /<Switch/);
  assert.doesNotMatch(settingsSource, /checked=\{traySettings\.enabled\}/);
  assert.doesNotMatch(settingsSource, /onChange=\{handleTrayEnabledChange\}/);
